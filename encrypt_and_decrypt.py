...
encrypt_and_decrypt_test() - 测试加密解密功能
...
def encrypt_and_decrypt_test(password="123456"):
    ...
    en_data = base64.b64encode(cipher_rsa.encrypt(password))
    print("加密数据信息：", type(en_data), '\n', len(en_data), '\n', en_data)

    # 加载公钥用于解密
    encoded_key = open("private.pem").read()
...
def rsa_sign(message, password="123456"):
    ...
    private_key = RSA.importKey(
        open("private.pem").read(), passphrase=password)
...
def rsa_signverify(message, signature):
    # 读取公钥信息用于验签
    public_key = RSA.importKey(open("pub.pem").read())
